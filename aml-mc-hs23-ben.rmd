...
path <- "C:/Users/Si Ben Tran/Documents/GitHub/fhnw-aml-Angewandtes-Machine-Learning/eda-dataexplorer-reports"
...
group_by(client_disp_card %>% filter(type.disp=="DISPONENT"), account_id) %>% summarise(n = n()) %>% arrange(desc(n))
...
order_account <- left_join(x = order, 
                          y = account,
                          by = "account_id") %>% 
  select(account_id, bank_to, account_to, amount, k_symbol) %>% 
  rename("bank_to.order" = "bank_to",
