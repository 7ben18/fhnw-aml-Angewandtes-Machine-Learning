...
roc_curves_list <- list(roc_list, roc_curves_rfc, roc_curves_dtc, roc_curves_full, roc_curves_base)

# Define the number of rows and columns for subplots
num_rows <- 2  # Change this to the desired number of rows
num_cols <- 3  # Change this to the desired number of columns

for (i in 1:length(model_names_list_string)) {
  # Set up the plotting grid for the current subplot
  par(mfrow = c(num_rows, num_cols))
  
  plot_roc(roc_curves = roc_curves_list[[i]], 
           roc_folds = FALSE, 
           roc_model = TRUE,
           model_name = model_names_list_string[[i]],
           get_agg_data = FALSE)
  
  # Reset the plotting layout for the next iteration
  par(mfrow = c(1, 1))
}

# plot from roc_curves_list every roc curve 
```
