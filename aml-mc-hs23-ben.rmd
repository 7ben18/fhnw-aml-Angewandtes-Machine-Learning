...
data <- data %>%
  left_join(
    x = non_buyers_fictive_date,
    y = data,
    by = "non_buyer_account_id"
  ) %>%
  select(-c(non_buyer_account_id, jaccard_value, buyer_account_id)) %>%
  rename(
    "issued.card" = "fictive_issued.card"
  ) %>%
  mutate(
    issued.card = ifelse(is.na(issued.card.x), issued.card.y, issued.card.x)
  ) %>%
  select(-c(issued.card.x, issued.card.y))
```
...
write_xlsx(data, "data_export/data.xlsx")
write_xlsx(buyers, "data_export/buyers.xlsx")
write_xlsx(non_buyers, "data_export/non_buyers.xlsx")
```
...
### 6.1.4 Funktion für Alter zur gegebenen issued.card Zeit
Wir brechnen das Alter des Kunden zur gegebenen Zeit. 
```{r}
buyers
```
...
## 6.4 Resultate der Kundeninformationen
```{r}
info_buyers
info_non_buyers
```
...
## 7.1 Funktionen zur Extraktion von Vermögen und Umsätze
```{r}
...
Ende Monat .> Vermoegen, aufpassen bei mehreren Transkationen (Reihefolge)
