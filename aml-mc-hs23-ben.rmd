...
# reorder col
buyers <- buyers %>%
  select(account_id, issued.card, date, year_month, type:k_symbol)
...
Weiter schauen wir uns an, wann der account erstellt wurde und berechnen 12 Monate in die "zukunft", damit wir Neukunden nicht als potentielle Kunden betrachten, da deren Verhalten am Anfang ein anderer ist, als bei einem langjaehrigen Kunden. 
```{r}
...
non_buyers <- non_buyers %>%
  select(account_id, date, year_month, type:k_symbol)
...
Wir erreichen diese mittels Iterationen durch jeden non_buyer_pivot. 
```{r}
