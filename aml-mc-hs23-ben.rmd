...
non_buyers <- gutschrift_belastung(non_buyers)
buyers <- gutschrift_belastung(buyers)
...
buyers_umsatz <- umsatz_end_month(buyers, display_df = TRUE)
non_buyers_umsatz <- umsatz_end_month(non_buyers, display_df = TRUE)
```
...
summarize_transactions <- function(df) {
  # Convert end_month to Date type
  ...
  df %>%
    ...
    summarize(
      ...
      spearman_balance = cor(balance.trans, as.numeric(end_month), method = "spearman", use = "complete.obs"),
      ...
      spearman_gutschrift = cor(total_gutschrift, as.numeric(end_month), method = "spearman", use = "complete.obs"),
      ...
      spearman_belastung = cor(total_belastung, as.numeric(end_month), method = "spearman", use = "complete.obs")
    )
}

# Call the function with your data frame (buyers_balance_umsatz)
summarize_transactions(buyers_balance_umsatz)
```

```{r}
summarize_transactions_last_three_months <- function(df) {
  # Convert end_month to Date type
  df <- df %>% mutate(end_month = ymd(paste0(end_month, "-01")))
  
  df %>%
    group_by(account_id.trans) %>%
    filter(end_month >= max(end_month) - months(2))
  
summarize_transactions_last_three_months(buyers_balance_umsatz)
```
