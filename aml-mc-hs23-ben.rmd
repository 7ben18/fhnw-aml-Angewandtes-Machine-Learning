...
```{r fig.width=10, fig.height=10, eda buyers}}
# loan?
...
```{r fiktives issued date in data}
...
```{r Transaktionen pro Jahr-Monat}
...
```{r Resultate der Kundeninformationen}
info_buyers
info_non_buyers
...
```{r Umsatz und Vermögen zusammenjoinen}
...
```{r Statistische Kennzahlen für Vermögen und Umsätze}
# Buyers
...
```{r Export/Import der Aggregationen}
...
```{r Kategorische Spalten One Hot Encoden}
...
```{r fig.height=6, fig.width=10, Anzahl Kreditkartenbesitzer}
...
```{r fig.height=6, fig.width=10, Verteilung der Kaufzeitpunkte}
full_data %>% 
...
```{r Data Leakage und Ratio Test Test}
...
```{r Baseline Model mit k-fold=10}
models <- list()
models_eval <- list()
...
```{r Funktion - Baseline Daten + kFolds}
...
```{r Funktion Baseline Model (Logistische Regression)}
...
```{r Funktion um Roc Kurve zu plotten}
...
Sie basiert auf der logistischen Funktion, die die Wahrscheinlichkeit berechnet, dass eine Beobachtung zu einer bestimmten Klasse gehört.
...
## 15.1 Baseline Model - Generalisierte lineare Modelle - Logistische Regression
```{r Baseline Model - Logistische Regression}
...
Hier in diesem Abschnitt schmeissen wir alle Variabeln aus unserem Dataframe herin mit dem Paramater `base_data = FALSE` und schauen uns, was das bringt. 

```{r Verbesserung des Baseline Modell}
...
```{r Andere Algorithmen - Decision Tree}
...
```{r Andere Algorithmen - Random Forest}
...
```{r Andere Optimierungen - Xgboost}
full_data_onehot
...
```{r Top-N Kunden Vergleich von Modellen}
...
```{r Altersverteilung der Top-N Kunden}
