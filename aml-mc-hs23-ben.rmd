...
### 10.1.6 Export / Import Full Data
```{r}
...
```{r}
all_objects <- ls()
remove_objects <- setdiff(all_objects, c("full_data", "full_data_onehot"))
rm(list = remove_objects)
print(ls())
```
...
Bevor wir mit der Modellierung fortfahren ist es noch wichtig zu wissen, wie viele Kundendaten wir noch besitzen, nachdem wir die Daten Preprocessed haben. Gesamthaft haben wir in unserem Datensatz 2841 Kunden die keine Karte besitzen. Diese Kunden sind potentielle Käufer einer Karte. ca. 4x kleiner, haben wir in unserem Datensatz 691 Käufer, bzw Kartenbesitzer. Wir haben somit einen unbalanced Dataset mit einem Verhältnis von 4:1 - Nicht-Käufer zu Käufer.
...
Im Density Plot erkennen wir, dass unsere fictive issued Daten durchaus ähnlich verteilt sind, wie die Kunden die eine Karte gekauft haben.
Die Anzahl an abgeschlossene Karten steigt im Verlaufe der Zeit, so auch unsere fictiven Issued Daten, was ein gutes Zeichen dafür ist, dass unsere Denkweise bzw. Vergleich von Buyer mit Non-Buyer funktioniert hat.
...
Die Funktion  ``create_baseline_data`` filtert uns die wichtigsten Features aus unserem grossen Datensatz. Diese Feature werden verwendet für unser Baseline Model. Die Features für das Baseline Model sind: Alter, Geschlecht, Region, Guthaben, Gutschrift und Belastung.

Auch erstellen wir direkt aus dem ganzen Datensatz die Daten Partitionierung bzw. unsere Kfolds, die wir durch createFolds erstellen.
...
data_partition <- createFolds(rest_data$has_card.card, k = num_folds)
...
Das glm Modell steht für Generalisierte Lineare Modelle und ist eine Erweiterung der klassischen linearen Modelle. Es ist ein flexibles statistisches Verfahren, das die Abhängigkeit einer stetigen Variablen von einer oder mehreren unabhängigen Variablen modelliert.

Ein hlm ermöglicht die Modellierung von abhängigen Variablen, die nicht notwendigerweise normalverteilt sind, und ermöglicht die Verwendung verschiedener Verteilungen. 

Für unsere Klassifikationstask nehmen wir die binomiale Verteilung, da wir eine binäre Klassifikation durchführen.
...
train_and_evaluate_lr <- function(train_index,
  ...
  if (test_mode) {
    # carefull! -train_index must be train + val index!
    train_data <- full_data[-train_index, ]
    test_data <- full_data[train_index, ]
    # use test_data like val_data
  ...
  #smoothed_roc <- smooth(roc_data, method = "binormal", n = 10)
  #auc <- auc(smoothed_roc)
  auc <- auc(roc_data)
  ...
  return(list(auc = auc, 
              roc_curve = roc_data,
              #smoothed_roc_curve = smoothed_roc,
              accuracy = accuracy, 
              precision = precision, 
              recall = recall,
              f1_score = f1_score,
              ...
              mcc = mcc,
              kappa = kappa,
              pred_prob = lr_validation_prediction))
}
...
perform_cross_validation <- function(data_partition, 
  ...
  for (fold in 1:num_folds) {
    result <- train_eval_function(data_partition[[fold]], 
                                  base_data=base_data, 
                                  test_mode=test_mode)
    #roc_curves[[fold]] <- result$smoothed_roc_curve
...
```{r}
plot_roc <- function(roc_curves, roc_folds = FALSE, roc_model = FALSE, model_name = "Baseline", get_agg_data = TRUE) {
  ...
  aggregated_plot <- aggregated_data %>%
    ...
    labs(title = "ROC Model - Durchschnittliche Performance", 
         x = "False Positive Rate", 
         y = "True Positive Rate",
         subtitle = model_name) + 
    theme_minimal() +
...
Die logistische Regression ist ein einfaches, aber weit verbreitetes Modell für binäre Klassifikationsprobleme.
Sie basiert auf der logistischen Funktion, die die Wahrscheinlichkeit berechnet, dass eine Beobachtung zu einer bestimmten Klasse gehört. Die Basis Grundlage ist die Sigmoid Funktion, der für eine Binäre Klasse die Wahrscheinlichkeit vorraussagt. 
Die logistische Regression kann leicht interpretiert werden und eignet sich gut für den Einstieg in Klassifikationsprobleme. 

## 15.1 Baseline Model - Logistische Regression - Logistische Regression
```{r}
cv_results_base <- perform_cross_validation(data_partition, 
                                            train_and_evaluate_lr, 
                                            base_data = TRUE,
                                            test_mode = FALSE)

length(cv_results_base$pred_prob)
...
result_base <- plot_roc(roc_curves_base,
                        roc_folds = TRUE, 
                        roc_model = TRUE,
                        model_name = "Baseline Modell",
                        get_agg_data = TRUE)
result_base$data
...
Hier in diesem Abschnitt schmeissen wir alle Variabeln aus unserem Dataframe in das Logistische Regression Modell hinein. Dies können wir tun indem wir mit dem Paramater `base_data = FALSE` setzen und schauen uns an was das bringt. 

```{r}
cv_results_full <- perform_cross_validation(data_partition, 
                                            train_and_evaluate_lr, 
                                            base_data = FALSE)
...
train_and_evaluate_dtc <- function(train_index, 
  ...
  dtc_model <- rpart(has_card.card ~ ., 
                     data = train_data, 
                     method = "class")
  
  # Prediction on the Validation Data
  dtc_validation_prediction <- predict(dtc_model, 
                                       newdata = validation_data, 
                                       type = "prob")
  
  # Create a data frame for prediction results
  dtc_pred_validation <- data.frame(has_card_prob = dtc_validation_prediction[, 2], 
                                    has_card_decision = as.factor(
                                      ifelse(
                                        dtc_validation_prediction[, 2] >= 0.5, 1, 0)
                                      ),
  ...
  roc_data <- roc(dtc_pred_validation$has_card.card, 
                  dtc_pred_validation$has_card_prob)
  auc <- auc(roc_data)
  
  # Calculate additional metrics
  confusion_matrix <- table(Predicted = dtc_pred_validation$has_card_decision, 
                            Actual = dtc_pred_validation$has_card.card)
  tp <- confusion_matrix[2, 2]
  tn <- confusion_matrix[1, 1]
  fp <- confusion_matrix[1, 2]
  fn <- confusion_matrix[2, 1]
...
```{r}
# Perform cross-validation with Decision Tree Classifier (DTC)
cv_results_dtc <- perform_cross_validation(data_partition, 
                                           train_and_evaluate_dtc,
                                           base_data=FALSE,
                                           test_mode = FALSE)

# Extract ROC curves and results for DTC
roc_curves_dtc <- cv_results_dtc$roc_curves
results_df_dtc <- cv_results_dtc$results_df
results_df_dtc_stats <- cv_results_dtc$results_df_stats

results_df_dtc_stats
results_df_dtc

# Plot ROC curves for DTC
result_dtc <- plot_roc(roc_curves_dtc, 
                       roc_folds = TRUE, 
                       roc_model = TRUE,
                       model_name = "Decision Tree Classifier",
                       get_agg_data = TRUE)

# Display ROC curve data for DTC
result_dtc$data
```


# 17 Andere Algorithmen - Random Forest

Zuerst schreiben wir die `train_and_evaluate` Funktion, diesmal statt mit der Logistische Regression, nutzen wir den Random Forest. Ansonsten belibt die Funktion in seiner Grundstruktur die selbe, hier hätte man durchaus code Wiederholungen verhindern können. Jedoch habe ich mich einfachhalts halber mal entschieden dies so zu lassen und zu einem späteren Zeitpunkt aufzuräumen, falls ich noch Zeit und Kapazität habe (stand 18.12.23).

Random Forest ist ein Ensemble-Lernverfahren, das auf Entscheidungsbäumen basiert. Es kombiniert mehrere Entscheidungsbäume, um robustere Klassifikationsmodelle zu erstellen. Das Ensemble ensteht, wenn am Ende alle Klassifikationsbäume eine Entscheidung getroffen haben und man die Klasse nimmt in dem die meisten Bäume übereinstimmen. Also sprich auch als Majority Voting bekannt.

```{r}
# Create a function to train the Random Forest model and evaluate
train_and_evaluate_rfc <- function(train_index, base_data=FALSE, test_mode=FALSE) {
...
```{r}
cv_results_rfc <- perform_cross_validation(data_partition, 
                                           train_and_evaluate_rfc,
                                           base_data=FALSE)
...
result_rfc <- plot_roc(roc_curves_rfc, 
                       roc_folds = TRUE, 
                       roc_model = TRUE,
...
# 18 Andere Optimierungen - Xgboost

Für das Xgboost nehmen wir den OneHotEncoded DataFrame, den wir im oberen Abschnitt vorbereitet haben. 
XGBoost (Extreme Gradient Boosting) ist ein leistungsstarker Machine-Learning-Algorithmus, der für Supervised Learning-Aufgaben wie Klassifikation und Regression verwendet wird. Er basiert auf dem Gradient Boosting-Prinzip und verwendet Entscheidungsbäume, die nacheinander trainiert werden, um die Vorhersagegenauigkeit zu verbessern, indem er auf vorherigen Fehlern aufbaut und Gewichtungen für die Modelle anpasst. Xgboost ist ein sehr beliebtes Modell und wir bei Klassifkation und Regressions Problemen oft angewendet. Der Nachteil hierbei ist jedoch, dass die Interpretierbarkeit des Modells schwieriger nachzuvollziehen ist, als bei einem Decision Tree.
...
X <- as.matrix(full_data_onehot %>% select_if(is.numeric) %>% select(-c(has_card.card)))
y <- full_data_onehot$has_card.card
...
for(i in 1:num_folds){
  ...
  dtrain <- xgb.DMatrix(data = X[train_idx, ],
                        label = y[train_idx])
  ...
  # get from a fold the confusion Matrix
  if (i == 1) {
    print("Konfusions Matrix für den ersten Fold")
  ...
  # ROC and AUC
  roc_obj <- roc(response = y[test_idx], predictor = pred_prob)
  auc <- auc(roc_obj)
  roc_curves_xgb[[i]] <- roc_obj
  
  # Store results
  xgb_results <- rbind(xgb_results, data.frame(Fold = i,
                                       AUC = auc,
                                       Accuracy = accuracy, 
                                       Precision = precision, 
                                       Recall = recall, 
...
# View aggregated results
print(xgb_results)
plot_roc(roc_curves = roc_curves_xgb, 
         roc_folds = TRUE, 
         roc_model = TRUE,
         model_name = "XGBoost",
         get_agg_data = TRUE)
...
# 19. Vergleich der Modelle
Vergleichen der Kandidatenmodelle und identifizieren des bzgl. 
Performance “besten” Modelles mit ROC, AUC und Precision.

(Metriken: Konfusionsmatrix, Accuracy,Kohen’s Kappa, Matthew’s Korrelation, Precision, Recall und F1 als Tabelle, ROC Kurve und AuC als Abbildung).

Unser Dataframe enthält zu allen erstellten Modellen deren Performance Leistung auf unterschiedlichen Metriken. Es hängt jeweils vom Use-Case ab, auf welcher dieser Metrik man optimieren möchte. In unserem Fall erkennen wir auch bei den diversen Barplots, dass nicht immer das gleiche Modell an der Spitze steht. 

Nutzen wir nun den AUC um zu beurteilen, welche unsere Modelle am besten perfort hat, so ist es das Xgboost Modell. Generell kann man aber jedoch sagen, dass die andere Modelle ebenfalls sehr gut performt haben und nicht weit weg vom AUC Wert des Xgboost liegen. Man könnte somit sich die Option offen halten, ob man eine Einbusse in der Performance in Kauf nimmt, um die Erklärbarkeit des ausgewählten Modells zu erhöhen. 

## 19.1 Modell Vergleich
```{r}
model_name_list <- list("XGBoost", "Random Forest", "Decision Tree", "Logistic Reg. Full", "Logistic Reg. Baseline")
model_results_list <- list(xgb_results, results_df_rfc, results_df_dtc, results_df_full, results_df_base)

for (i in 1:length(model_name_list)) {
  model_results_list[[i]]$model_name <- model_name_list[[i]]
}


for (i in 1:length(model_results_list)) {
  model_results_list[[i]] <- model_results_list[[i]] %>%
    group_by(model_name) %>%
    summarise(
      mean_auc = mean(AUC),
      sd_auc = sd(AUC),
      ...
      mean_f05_score = mean(f05_score),
      sd_f05_score = sd(f05_score),
      mean_f2_score = mean(f2_score),
      sd_f2_score = sd(f2_score),
      mean_mcc = mean(mcc),
      sd_mcc = sd(mcc),
...
model_results_df <- do.call(rbind, model_results_list)

model_results_df

create_performance_plot <- function(data, x_col, y_col, 
                                    y_sd_col, title, 
                                    x_label, y_label) {
  
  model_colors = c("XGBoost" = "red", 
  "Random Forest" = "blue", 
  "Decision Tree" = "green", 
  "Logistic Reg. Full" = "orange", 
  "Logistic Reg. Baseline" = "purple")
  
  ggplot(data, aes(x = reorder({{x_col}}, {{y_col}}), y = {{y_col}}, fill = {{x_col}})) +
    geom_bar(stat = "identity") +
    geom_errorbar(aes(ymin = {{y_col}} - {{y_sd_col}}, ymax = {{y_col}} + {{y_sd_col}}), width = 0.2) +
    coord_flip() +
    labs(
      title = title,
      x = x_label,
...
model_results_df %>%
  create_performance_plot(
    ...
    y_col = mean_auc,
    y_sd_col = sd_auc,
    title = "AUC Performance der Modelle",
    x_label = "Model",
    y_label = "AUC"
  )
...
model_results_df %>%
  create_performance_plot(
    x_col = model_name,
    y_col = mean_accuracy,
    y_sd_col = sd_accuracy,
    title = "Accuracy Performance der Modelle",
    x_label = "Model",
    y_label = "Accuracy"
  )
...
model_results_df %>%
  create_performance_plot(
    x_col = model_name,
    y_col = mean_precision,
    y_sd_col = sd_precision,
    title = "Precision Performance der Modelle",
    x_label = "Model",
    y_label = "Precision"
  )
...
model_results_df %>%
  create_performance_plot(
    ...
    y_col = mean_recall,
    y_sd_col = sd_recall,
    title = "Recall Performance der Modelle",
    x_label = "Model",
    y_label = "Recall"
  )
...
model_results_df %>%
  create_performance_plot(
    x_col = model_name,
    y_col = mean_f1_score,
    y_sd_col = sd_f1_score,
    title = "f1 Performance der Modelle",
    x_label = "Model",
    y_label = "f1-score"
  )
...
model_results_df %>%
  create_performance_plot(
    ...
    y_col = mean_kappa,
    y_sd_col = sd_kappa,
    title = "Kappa Performance der Modelle",
    x_label = "Model",
    y_label = "Kappa"
  )
...
model_results_df %>%
  create_performance_plot(
    ...
    y_col = mean_mcc,
    y_sd_col = sd_mcc,
    title = "MCC Performance der Modelle",
    x_label = "Model",
    y_label = "MCC"
  )


# Define a list of metrics you want to create plots for
metrics <- c("AUC", "Accuracy", "Precision", "Recall", "f1", "Kappa", "MCC")

# Create a for loop to generate plots for each metric
for (metric in metrics) {
  metric_col <- switch(
    metric,
    "AUC" = c("mean_auc", "sd_auc"),
    "Accuracy" = c("mean_accuracy", "sd_accuracy"),
    "Precision" = c("mean_precision", "sd_precision"),
    "Recall" = c("mean_recall", "sd_recall"),
    "f1" = c("mean_f1_score", "sd_f1_score"),
    "Kappa" = c("mean_kappa", "sd_kappa"),
    "MCC" = c("mean_mcc", "sd_mcc")
  )
  
  plot_title <- paste(metric, "Performance der Modelle")
  y_label <- metric
...
model_names_list_string <- c("XGBoost", "Random Forest", "Decision Tree", "Logistic Reg. Full", "Logistic Reg. Baseline")
roc_curves_list <- list(roc_curves_xgb, roc_curves_rfc, roc_curves_dtc, roc_curves_full, roc_curves_base)

for (i in 1:length(roc_curves_list)) {
  plot_roc(roc_curves_list[[i]], 
           roc_folds = FALSE, 
           roc_model = TRUE,
           model_name = model_names_list_string[[i]],
           get_agg_data = FALSE)
}
...
## 20.1 Top-N Kunden Vergleich von Modellen

Eine Möglichkeit Top-N Kunden aus der Modellierung zu erhalten, ist es mit der Wahrscheinlichkeits Vorhersage des Modelles zu arbeiten. In unserem Fall, haben wir oben beim XGBoost Modell die variable `pred_prob` definiert. Diese wird jedoch leider nach jedem kfold wieder überschrieben, da diese sich in einer for Schleife befindet. Wir arbeiten trotzem einfachhaltshalber damit, müssen uns jedoch bewusst sein, dass es sich hierbei um den letzten kfold handelt und bei einem anderen Fold es nicht die identischen Resultate geben wird. Wir haben bei der Funktion `create_top_customer_df` die Möglichkeit die Anzahl der Top-N Kunden zu definieren, hierbei werden nur die Top-N Kunden als Dataframe zurückgegeben, welche noch keine Kreditkarte haben, aber das Modell diese vorhhersagt.

```{r}
create_top_customer_df <- function(data_vector, pred_prob_vector, full_data, n_customer = NULL) {
  # Create a dataframe from the two vectors
  top_customer_df <- data.frame(account_id = data_vector, pred_prob = pred_prob_vector) %>%
    arrange(desc(pred_prob)) 
  print(top_customer_df)
  
  # if n_customer is not defined, return all customers
  if (is.null(n_customer)) {
    n_customer = length(data_vector)
  }
  
  # Inner join with full data
  full_data_top_customer <- inner_join(full_data, 
                                       top_customer_df, 
                                       by = "account_id")  %>% 
    filter(has_card.card == 0) %>% arrange(desc(pred_prob)) %>% 
    head(n_customer) 
  
  return(full_data_top_customer)
}

top15_customer <- create_top_customer_df(folds[[10]], 
                                         pred_prob, 
                                         full_data_onehot,
                                         n_customer = 15)
top15_customer
...
### 20.1.1 Top-N Kunden Gemeinsamkeiten?
Interessant ist es nun zu wissen, welche Gemeinsamkeiten diese Kunden haben, die keine Karte besitzen, unser Modell jedoch mit einer grossen Wahrscheinlichkeit vorhersagt, dass diese Kunden geeignet seien eine Karte zu besitzen. Hier eine kleine Explorative Analyse der Top-N Kunden.


### 20.1.2 Altersverteilung der Top-N Kunden

Unsere potentiellen TopN Kunden sind Kunden die sich gegen das Alter von 50 Jahren bewegen.
```{r}
# distruitbio plot of age with geom_density
ggplot(top15_customer, aes(x = age.client)) +
  geom_density(alpha = 0.5) +
  labs(
    title = "Verteilung der Top-N Kunden nach Alter",
    x = "Alter",
    y = "Dichte"
  ) +
  theme_minimal()
```
### 20.1.2 Geschlechterverteilung der Top-N Kunden

Hier sehen wir, dass potentielle Kunden eher Frauen sind als Männer da sie häufiger vertreten sind. Dies könnte aber jedoch auch ein Bias von unserem Datensatz sein, welches man genauer untersuchen müsste.
```{r}
ggplot(top15_customer, aes(x = gender.clientmale)) +
  geom_bar() +
  labs(
    title = "Verteilung der Top-N Kunden nach Geschlecht",
    subtitle = "1 = Mann, 0 = Frau",
    x = "Geschlecht",
    y = "Anzahl"
  ) +
  theme_minimal()
...
# 21. Wichtigkeit und Balancierung
...
```{r}
explainer_xgb <- explain(model = bst, 
                         data = X, 
                         y = y, 
                         label = "XGBoost")
```
...
```{r}
pfi_plots <- function(N=100, n_feature_show=50) {
  set.seed(123)
  pfi_xgb <- model_parts(explainer_xgb, 
                        type = "variable_importance", 
                        N = N)
  
  p <- plot(head(pfi_xgb, n_feature_show)) + 
    ggtitle(paste("N =", N)) + 
      theme_minimal() + 
...
pfi_plots(N = 50, n_feature_show = 25)
pfi_plots(N = 100, n_feature_show = 25)
pfi_plots(N = NULL, n_feature_show = 25)
```

## 21.2 Reduzieren des Modells

Wir reduzieren das XGboost Modell, indem wir Parameter anpassen bzw. entfernen oder den Wert verringern. 

Wir können erkennen, dass bei der Reduktion des Xgboost Modelles, sich die Performance leicht verbessert. Jedoch kann man diese Verbesserung auch vernachlässigen, da sich die Standardabweichungen vom Mittelwert nicht gross verändern und die Überlappung somit sehr gross ist. 


```{r}
xgb_results_red <- data.frame()
roc_curves_xgb_red <- list()
...
for(i in 1:num_folds){
  ...
  roc_obj <- roc(response = y[test_idx], predictor = pred_prob)
  auc <- auc(roc_obj)
  roc_curves_xgb_red[[i]] <- roc_obj
  
  # Store results
  xgb_results_red <- rbind(xgb_results, data.frame(Fold = i,
                                       AUC = auc,
                                       Accuracy = accuracy, 
                                       Precision = precision, 
                                       Recall = recall, 
...
# View aggregated results
print(xgb_results_red)

```
```{r}
# create a column with the model name 
xgb_results_red$model_name <- "XGBoost Reduziert"

# aggregate by model_name
xgb_results_red_agg <- xgb_results_red %>%
  group_by(model_name) %>%
  summarise(
    mean_auc = mean(AUC),
    sd_auc = sd(AUC),
...
# combine df with the other models
model_results_df_with_red <- rbind(model_results_df, xgb_results_red_agg)

model_results_df_with_red

# visualize xgboost with xgboost reduced
model_results_df_with_red %>% filter(model_name %in% c("XGBoost", "XGBoost Reduziert")) %>%
  ggplot(aes(x = reorder(model_name, mean_auc), y = mean_auc, fill = model_name)) +
  geom_bar(stat = "identity") +
...
# 22. Finales Model beschreiben

Das Finale Modell ist unser XGboost Modell, welches für Personen die Keine Kreditkarte besitzen eine Wahrscheinlichkeit vorhersagt, ob diese eine Kreditkarte besitzen sollten.

Der Mehrwert dieses Modell für die Bank ist, dass diese potentielle Kunden ansprechen kann die noch keine Kundenkarte besitze und die Bank somit ihre Kundenbasis erweitern kann und mehr Umsatz genereiert. 

Weiter kann die Bank dieses Modell als einen Part der Marketing-Kampagne der Bank verwenden. Eventuell lassen sich mehrere Strategien miteinander kombinieren um den best möglichen potentiellen Kunden ausfindig zu machen und diesen dann gezielt anzusprechen, um diesen als Kunden zu gewinnen.
