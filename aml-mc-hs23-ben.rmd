...
#data_partition[["Fold01"]]
```




## 14.2 Funktion for Training Logistic Regression Model
```{r}
train_and_evaluate_lr <- function(train_index) {
  ...
  confusion_matrix <- table(Predicted = lr_pred_validation$has_card_decision, Actual = lr_pred_validation$has_card.card)
...
## 14.3 Perform Cross-Validation on Base Dataset
```{r}
# initalize empty list and df
roc_curves <- list()
...
for (fold in 1:num_folds) {
  result <- train_and_evaluate_lr(data_partition_base[[fold]])
...
# plot roc curve
ggplot(plot_data, aes(x = FPR, y = TPR, color = Fold)) +
  geom_line() +
  labs(title = "ROC Curves for Each Fold", x = "False Positive Rate", y = "True Positive Rate") +
  theme_minimal() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray")
```


# 15 Random Forest Model - Andere Algorithmen 
...
## 15.1 Random Forest Model mit Baseline Feautres
```{r}
create_baseline_data <- function(data) {
  baseline_data <- data %>%
    select(has_card.card, age.client, gender.client, region.district, mean_balance, mean_gutschrift, mean_belastung) %>%
...
## 15.2 Partitionierung in K-Folds

Auch hier habe ich einen bessere Funktion gefunden, die mir die Arbeit abnimmt. 
Damit wir die Partitionierung reproduzieren können, setzen wir den Seed auf 123 und num_folds auf 10. 
Die Partitionierung gibt mir die Indexe des Dataframes zurück, die ich dann in der nächsten Funktion verwenden kann.
```{r}
set.seed(123)
num_folds <- 10
...
## 15.3 Train and Evaluation Funktion für Baseline Model
...
## 15.4 Resultate von jedem Fold in einem Dataframe speichern

Wir führen die Funktion `train_and_evaluate` für jeden Fold aus und speichern die Resultate in einem Dataframe sowie die ROC Kurven in einer Liste, um sie später zu plotten.
```{r}
# initalize empty list and df
roc_curves <- list()
results_df <- data.frame(Fold = integer(), 
...
# Perform cross-validation
for (fold in 1:num_folds) {
  result <- train_and_evaluate_rfc(data_partition_base[[fold]])
  roc_curves[[fold]] <- result$roc_curve
  results_df <- rbind(results_df, data.frame(Fold = fold, 
                                             AUC = result$auc, 
                                             Accuracy = result$accuracy, 
...
perform_cross_validation <- function(data_partition) {
    # Initialize empty list and dataframe for storing results
    roc_curves <- list()
    results_df <- data.frame(Fold = integer(), 
                             AUC = numeric(), 
                             Accuracy = numeric(), 
    ...
    # Perform cross-validation
    for (fold in 1:num_folds) {
        result <- train_and_evaluate_rfc(data_partition[[fold]])
        roc_curves[[fold]] <- result$roc_curve
        results_df <- rbind(results_df, data.frame(Fold = fold, 
                                                   AUC = result$auc, 
                                                   Accuracy = result$accuracy, 
    ...
    # Return results
    list(ResultsDataFrame = results_df, ROC_Curves = roc_curves)
}


# Assuming `data_partition_base` and `num_folds` are already defined
cross_val_results_base <- perform_cross_validation(data_partition_base)
results_df <- cross_val_results_base$ResultsDataFrame
roc_curves <- cross_val_results_base$ROC_Curves

cross_val_results_full <- perform_cross_validation(data_partition_full)
results_df <- cross_val_results_full$ResultsDataFrame
roc_curves <- cross_val_results_full$ROC_Curves
...
# plot roc curve
ggplot(plot_data, aes(x = FPR, 
                      y = TPR, 
                      color = Fold)) +
  geom_line() +
  labs(title = "ROC Curves for Each Fold", 
       x = "False Positive Rate", 
       y = "True Positive Rate") +
  theme_minimal() +
  geom_abline(slope = 1, 
...
# 15. Verbesserung des Baseline Modell
...
## 15.2 Andere Algorithmen 
```{r}

```

## 15.3 Andere Optimierungen
```{r}
