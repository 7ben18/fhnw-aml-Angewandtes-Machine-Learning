...
# 14. Verbesserung des Baseline Modell
...
for (i in 1:length(kfolds_data)) {
    ...
    # Fit the logistic regression model for the current fold
    full_linear_model <- glm(has_card.card ~ ., data = train_data,
                           family = binomial)
    
    # Store the model in the list
    models[[i]] <- full_linear_model
    
    # perform evaluation with model_evlaution function
    models_eval[[i]] <- model_evaluation(full_linear_model, val_data) 
    models_eval[[i]]$full_linear_model_nr = i
    
}
```

```{r}
full_linear_model_results <- do.call(rbind, models_eval)

full_linear_model_results %>% ggplot(aes(x = FPR, y = TPR, color = as.factor(full_linear_model_nr))) + 
  geom_line() +
        coord_cartesian(xlim = c(0, 1), ylim = c(0, 1)) +
        geom_abline(intercept = 0, slope = 1, linetype = "dashed") +
        labs(
  ...
  scale_color_discrete(name = "Kfold Full Linear Models")

full_linear_model_results %>%  group_by(threshold) %>%
  summarize(
    threshold = first(threshold),
    mean_accuracy = mean(accuracy, na.rm = TRUE),
    sd_accuracy = sd(accuracy, na.rm = TRUE),
...
## 14.2 Andere Algorithmen 
```{r}

```

## 14.3 Andere Optimierungen
```{r}
...
## 16.1 Top-N Kunden Vergleich von Modellen
```{r}

```

 
# 17. Wichtigkeit und Balancierung
