...
buyers$month <- format(as.Date(buyers$date), "%m")


# reorder col
buyers <- buyers %>%
  select(account_id, issued.card, date, month, type:k_symbol)
...
Da wir nun uns in einem richtigen Roll-up Fenster befinden fuer die Kunden extrahieren wir nun den Monat und fuegen diese als weitere Spalte hinzu. Weiter fügen wir dann eine Spalte value = 1, damit das pivotieren einfacher funktioniert und wir von dem Monat die Anzahl Transkationen kennen, Parallel dazu aggregieren wir noch die balance und amount und erstellen daraus Features für den spezifischen Account. Dadurch haben wir für eine Zeile einen account_id und können diese nun mit unserem grossen Dataframe aus 2. inner_joinen und erhalten die Attribute von Käufer. Einige Attribute sind fuer den Vergleich irrelevant und werden nicht beruecksichtigt.

```{r}
aggregate_buyers_data <- function(buyers, data) {
  # aggregate_buyers_data aggregiert die numerischen transkationsdaten indem diese
  # nach account_id gruppiert werden + pivotieren nach Monate.
  
  # buyers: Käufer oder fikitive Käufer!
  # data: Das Grosse DataFrame mit account als row
  # return: joined Dataframe, 1 row = 1 account! 
  
  # Group by account_id and calculate aggregations
  buyers_grouped <- buyers %>% 
    group_by(account_id) %>% 
    summarise(
  ...
  buyers_pivot <- buyers %>% 
    select(c(account_id, month)) %>% 
    mutate(value = 1) %>% 
    group_by(account_id, month) %>% 
    summarise(count = n(), .groups = "drop") %>% ungroup() %>% 
    right_join(buyers_grouped, by = "account_id") %>% 
    pivot_wider(
      names_from = month,
      values_from = count,
  ...
  buyers_data <- inner_join(x = buyers_pivot, y = data, by = "account_id")
...
# call function to aggregate transaktional data from buyers and concat them to big dataset
buyers_data <- aggregate_buyers_data(buyers, data)

# display big Dataset from buyers
buyers_data 
```
...
#####
# zu account_ids_no_cards fiktive issued.card hinzufuegen, indem vergleich zwischen buyer und non_buyer gezogen wird!
...
# extract month
non_buyers$month <- format(as.Date(non_buyers$date), "%m")
...
# inner join with data
non_buyers_data <- inner_join(x = non_buyers_pivot, y = data, by = "account_id")
...
Anschliessend mit 1 oder 0 auffuellen. 

Analog mache ich das gleiche fuer non buyers, filtere aber jedoch nicht das Fenster. 
Anschliessend picke ich random einen buyer aus und nehme den grössten jaccard wert mit non buyer in bezug auf die transkationsdaten jahr monat.
