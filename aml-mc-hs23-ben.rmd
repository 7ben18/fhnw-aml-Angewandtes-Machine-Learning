...
account_ids_has_card <- data %>%
  ...
  rename("kaufdatum.card" = "issued.card")
...
Das Rollup Fenster definieren wir indem wir vom Kaufdatum 13 Monate subtrahieren (12 Monate History + 1 Monat Lag)
```{r}
# vom Kaufdatum -13 Monate von Kunden die eine Karte besitzen
account_ids_has_card <- account_ids_has_card %>%
  mutate(start_date = kaufdatum.card %m-% months(13))
...
trans_has_card_within_range <- inner_join(
  x = account_ids_has_card,
  y = trans,
  by = "account_id"
) %>% 
  filter(date >= start_date & date <= kaufdatum.card )

# check transaction within range
trans_has_card_within_range

```

```{r}
# create a function where you filter by account 
trans_has_card_within_range %>% filter(account_id %in% c(1, 2))
...
visualize_account_balance <- function(transactions, 
  ...
  plot <- ggplot(trans_with_rolling_mean, aes(x = date)) +
    ...
    geom_vline(xintercept = as.numeric(unique(filtered_transactions$kaufdatum.card)),
...
visualize_account_balance(trans_has_card_within_range, 
                          accounts_to_filter = c(7, 97), 
                          window_size = 30)
