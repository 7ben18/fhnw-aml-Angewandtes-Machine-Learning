...
### 6.1.3 Funktion Kategorische One-Hot Encoding
```{r}
one_hot_encode <- function(data_frame, col_name) {
  ...
  data_frame[[col_name]][is.na(data_frame[[col_name]])] <- "Unknown"
  
  # Convert the column to a factor
  data_frame[[col_name]] <- as.factor(data_frame[[col_name]])
  
  # One-hot encode the column
  one_hot <- model.matrix(~ data_frame[[col_name]] - 1)
  
  # Convert the matrix to a data frame
  one_hot_df <- as.data.frame(one_hot)
  
  # Bind the one-hot encoded data frame to the original data frame (excluding the original column)
  new_data_frame <- cbind(data_frame[, !(names(data_frame) == col_name)], one_hot_df)
...
extract_count(buyers, trans)
```


```{r}
# One Hot Encode type.trans
one_hot_encode(buyers, "type.trans")
```

## 6.3 Kundeninformation für 12 Monate von Nicht-Käufer
```{r}
# extract number of transaction for year_month
extract_count(non_buyers, trans)
```
