...
## 13.1 Baseline Daten
```{r}
train_data %>% select(has_card.card, age.client, gender.client, region.district, mean_balance, mean_gutschrift, mean_belastung) %>% arrange(desc(has_card.card))
```
...
```{r}
baseline_evaluation <- model_evaluation(base_line_model, test_data)
baseline_evaluation
