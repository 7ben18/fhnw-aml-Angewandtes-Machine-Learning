...
data_partition <- createFolds(baseline_train_data$has_card.card, k = num_folds)
...
perform_cross_validation <- function(data_partition, 
  ...
  for (fold in 1:num_folds) {
    result <- train_eval_function(data_partition[[fold]], base_data=base_data)
...
model_results_df %>%
  ggplot(aes(x = reorder(model_name, mean_precision), y = mean_precision, fill = model_name)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin = mean_precision - sd_precision, ymax = mean_precision + sd_precision), width = 0.2) +
  coord_flip() +
  labs(
...
```{r}
create_top_customer_df <- function(data_vector, pred_prob_vector, full_data, n_customer = 15) {
...
### 20.1.3 Transaktionsdaten der Top-N Kunden
```{r}
# make grid plots of mean_balance and mean_gutschrift, mean_belastung
grid.arrange(
  ggplot(top15_customer, aes(x = mean_balance)) +
    geom_density(alpha = 0.5) +
    labs(
      title = "Verteilung der Top-N \nKunden nach \nBalance",
      x = "Balance",
      y = "Dichte",
      
    ),
  
  ggplot(top15_customer, aes(x = mean_gutschrift)) +
    geom_density(alpha = 0.5) +
    labs(
      title = "Verteilung der Top-N \nKunden nach \nGutschrift",
      x = "Gutschrift",
  ...
  ggplot(top15_customer, aes(x = mean_belastung)) +
    geom_density(alpha = 0.5) +
    labs(
      title = "Verteilung der Top-N \nKunden nach \nBelastung",
      x = "Belastung",
...
### 20.1.4 Transaktionsdaten der Top-N Kunden der letzten 3 Monate
```{r}
grid.arrange(
  ggplot(top15_customer, aes(x = mean_balance_last_3_month)) +
    geom_density(alpha = 0.5) +
    labs(
      title = "Verteilung der Top-N \nKunden nach \nBalance",
      x = "Balance",
  ...
  ggplot(top15_customer, aes(x = mean_gutschrift_last_3_month)) +
    geom_density(alpha = 0.5) +
    labs(
      title = "Verteilung der Top-N \nKunden nach \nGutschrift",
      x = "Gutschrift",
  ...
  ggplot(top15_customer, aes(x = mean_belastung_last_3_month)) +
    geom_density(alpha = 0.5) +
    labs(
      title = "Verteilung der Top-N \nKunden nach \nBelastung",
      x = "Belastung",
