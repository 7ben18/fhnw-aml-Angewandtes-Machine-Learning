...
```{r}
# Loan und Account, nicht jeder Account hat einen Kredit.

print(paste("Anzahl Zeilen loan df:", nrow(loan)))
print(paste("Anzahl Zeilen account:", nrow(account)))


account_loan <- left_join(x = account,
                          y = loan, 
                          by = "account_id") %>% 
  mutate(has_loan = !rowSums(is.na(.))) %>% 
  select(-c(loan_id)) %>% 
  rename("account_creation_date" = "date.x",
         "granted_loan_date" = "date.y")

print(paste("Anzahl Zeilen account_loan df:", nrow(account_loan)))
account_loan %>% sample_n(size = 5)
...
```{r}
# Clienten die keinen Account haben? 
client_disp_card_district
```
